...
<aside id="sidebar" class="fixed left-0 top-0 z-40 flex h-screen flex-col overflow-hidden bg-gradient-to-b from-[#0F2557] via-[#1E3A8A] to-blue-600 shadow-2xl">
    ...
    <div class="flex h-16 items-center justify-between border-b border-white/10 px-4">
        ...
        <button onclick="toggleSidebar()" class="flex h-8 w-8 items-center justify-center rounded-lg bg-white/10 text-white/70 hover:bg-white/20 transition">
            <svg id="toggleIcon" class="h-3.5 w-3.5 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        <?php if ($role === 'admin' || $role === 'hr'): ?>
            <p class="px-3 text-[9px] font-bold uppercase tracking-widest text-white/30 sidebar-text">Menu Utama</p>

            <a href="<?= BASE_URL ?>views/dashboard.php" class="nav-item flex items-center gap-3 rounded-xl px-3 py-2 transition <?= isActive('dashboard.php') ?>">
            ...
            <?php if ($role === 'admin'): ?>
                ...
                <a href="<?= BASE_URL ?>views/divisi/index.php" class="nav-item flex items-center gap-3 rounded-xl px-3 py-2 transition <?= isActive('divisi') ?>">
                    <svg class="h-[18px] w-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <?php if ($role === 'candidate'): ?>
            ...
            <p class="px-3 pt-3 text-[9px] font-bold uppercase tracking-widest text-white/30 sidebar-text">Profil</p>
            <a href="<?= BASE_URL ?>views/candidate/profile.php?id=<?= $candidateData['id'] ?? '' ?>" class="nav-item flex items-center gap-3 rounded-xl px-3 py-2 transition <?= isActive('profile.php') ?>">
                <svg class="h-[18px] w-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
                <span class="sidebar-text">Profil Saya</span>
    ...
    <div class="flex items-center gap-3 border-t border-white/10 px-3 py-3">
        <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-400 text-xs font-bold text-white uppercase">
            <?php
            if ($_SESSION['role'] == 'admin') echo substr($_SESSION['username'] ?? 'U', 0, 2);
            elseif ($_SESSION['role'] == 'hr') echo substr($userData['nama_staff'] ?? 'U', 0, 2);
            else echo substr($candidateData['nama_lengkap'] ?? 'U', 0, 2);
            ?>
        </div>
        <div class="sidebar-text">
            <p class="text-xs font-semibold text-white">
                <?php
                if ($_SESSION['role'] == 'admin') echo $_SESSION['username'] ?? 'Admin';
                elseif ($_SESSION['role'] == 'hr') echo $userData['nama_staff'] ?? 'Staff';
                else echo $candidateData['nama_lengkap'] ?? 'User';
                ?>
            </p>
            <p class="text-[10px] text-blue-300 uppercase"><?= $_SESSION['role'] ?></p>
        </div>
    </div>
